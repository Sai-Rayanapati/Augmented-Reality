...
// Anaglyph control
static float ipd = 1.5f;				// Distance between left/right eye.
...
static bool renderCustomScene = false;
std::vector<glm::mat4> treeTransforms;
Model treeModel(20);
Ground ground;

static void generateCustomScene(){
    treeTransforms.clear();

    treeModel.LoadModelInstanced("../src/models/tree/Tree.obj", "../src/models/tree");

    for (int i = 0; i < 20; ++i) {
        glm::mat4 modelMatrix = glm::mat4();
        glm::vec3 translation = glm::vec3(glm::linearRand(-50.0f, 50.0f), -30.0f, glm::linearRand(-50.0f, 50.0f));
        modelMatrix = glm::translate(modelMatrix, translation);
        modelMatrix = glm::scale(modelMatrix, glm::vec3(12, 12, 12));
        treeTransforms.push_back(modelMatrix);
    }
}
...
int main(void)
{
    ...
    ground.initialize(glm::vec3(0.0f,-30.0f,0.0f), glm::vec3(200.0f,0.5f,200.0f));
        ...
        if (anaglyphMode == None) {
            ...
            if (renderCustomScene){
                ...
                for (const auto& transform : treeTransforms)
                    treeModel.DrawInstanced(transform, viewMatrix, projectionMatrix, treeTransforms);
            }
            ...
            glm::mat4 vpLeft;
            glm::mat4 vpRight;
            glm::mat4 viewMatrixLeft;
            glm::mat4 viewMatrixRight;
            ...
            if (anaglyphMode == ToeIn) {
                ...
                glm::vec3 leftEyePosition = eyeCenter - right * (3 * ipd / 2.0f);
                glm::vec3 rightEyePosition = eyeCenter + right * (3 * ipd / 2.0f);

                // Left and Right view matrices
                viewMatrixLeft = glm::lookAt(leftEyePosition,lookat, up);
                viewMatrixRight = glm::lookAt(rightEyePosition,lookat, up);
                ...
                // Calculate the frustum shift
                float aspectRatio = (float)windowWidth / (float)windowHeight;
                float halfScreenWidth = zNear * tan(glm::radians(FoV) / 2.0f);
                float frustumShift = (ipd / 2.0f) * aspectRatio * (zNear / viewDistance);

                // Left Eye Projection Matrix
                float left = -aspectRatio * halfScreenWidth - frustumShift;
                float right = aspectRatio * halfScreenWidth - frustumShift;
                float top = halfScreenWidth;
                float bottom = -halfScreenWidth;
                projectionMatrixLeft = glm::frustum(left, right, bottom, top, zNear, zFar);

                // Right Eye Projection Matrix
                left = -aspectRatio * halfScreenWidth + frustumShift;
                right = aspectRatio * halfScreenWidth + frustumShift;
                projectionMatrixRight = glm::frustum(left, right, bottom, top, zNear, zFar);

                // Adjust eye positions for left and right eye
                glm::vec3 forward = glm::normalize(lookat - eyeCenter);
                glm::vec3 rightDirection = glm::normalize(glm::cross(forward, up));

                glm::vec3 leftEyePosition = eyeCenter - rightDirection * (ipd / 2.0f);
                glm::vec3 rightEyePosition = eyeCenter + rightDirection * (ipd / 2.0f);
            ...
            if (renderCustomScene){
                if (anaglyphMode == ToeIn){
                    for (const auto& transform : treeTransforms)
                        treeModel.DrawInstanced(transform, viewMatrixLeft, projectionMatrix, treeTransforms);
                }
                else if (anaglyphMode == Asymmetric)
                    for (const auto& transform : treeTransforms)
                        treeModel.DrawInstanced(transform, viewMatrixLeft, projectionMatrixLeft, treeTransforms);
            }
            ...
            if (renderCustomScene){
                if (anaglyphMode == ToeIn){
                    for (const auto& transform : treeTransforms)
                        treeModel.DrawInstanced(transform, viewMatrixRight, projectionMatrix, treeTransforms);
                }
                else if (anaglyphMode == Asymmetric)
                    for (const auto& transform : treeTransforms)
                        treeModel.DrawInstanced(transform, viewMatrixRight, projectionMatrixRight, treeTransforms);
            }
