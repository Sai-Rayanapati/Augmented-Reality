...
const char* buildingTextures[] = {"../lab2/facade1.jpg", "../lab2/facade3.jpg","../lab2/facade0.jpg", "../lab2/facade4.jpg"};
...
struct Building {
	...
	void initialize(glm::vec3 position, glm::vec3 scale, const char *texture_file_path) {
		...
		// --------------------------------------------------------
        // --------------------------------------------------------
        for (int i = 0; i < 24; ++i) uv_buffer_data[2*i+1] *= this->scale.y/20;
        ...
        // --------------------
        // --------------------
        textureID = LoadTexture(texture_file_path);
...
static float randomHeight() {
	float r = 20 + (rand() % 7) * 10;
	return r;
}

static const char* getRandomTexture() {
    int index = rand() % 4;
...
int main(void)
{
    ...
    for (int i = -160; i <= 160; i += 40) { // Positions along the x-axis
        for (int j = -46; j <= 46; j += 92) { // Two rows along the z-axis
            Building b;
            glm::vec3 position = glm::vec3(i, 0, j);
            glm::vec3 scale = glm::vec3(16, randomHeight(), 16);
            const char* texturePath = getRandomTexture();

            b.initialize(position, scale, texturePath);
    ...
    Ground gr;
    gr.initialize(glm::vec3(0, 0, 0), glm::vec3(200, 0.2, 30),"../lab2/road.jpg");
    ground.push_back(gr);
    gr.initialize(glm::vec3(0, 0, 60), glm::vec3(200, 0.2, 30),"../lab2/soil.jpg");
    ground.push_back(gr);
    gr.initialize(glm::vec3(0, 0, -60), glm::vec3(200, 0.2, 30),"../lab2/soil.jpg");
    ground.push_back(gr);
